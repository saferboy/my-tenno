...
function formatRemaining(ms: number, dayLabel: string): string {
  ...
  return days > 0 ? `${days} ${dayLabel} ${clock}` : clock
}
...
function ResetCountdown({ label, target }: ResetCountdownProps): React.JSX.Element {
  ...
  return (
    <div className="surface-base rounded-lg p-4">
      ...
      <p className="mt-1 font-mono text-xl text-[var(--color-tenno-cyan)]">
        {formatRemaining(target - now, t('resetCountdown.day'))}
      </p>
    </div>
  )
}
